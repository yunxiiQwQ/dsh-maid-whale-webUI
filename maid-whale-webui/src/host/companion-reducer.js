...
import { activityCopy, activityStage, statusCopy, taskCopy } from './status-copy.js'
...
function toolCallIdOf(event, fallback = '') {
  ...
  const contentCallId = Array.isArray(content) ? content.find((item) => item?.toolCallId)?.toolCallId : undefined
  return String(
...
function isUserQuestionTool(name) {
  ...
  const hasNounFromUser = tokens.some(
    (token, index) => nouns.has(token) && tokens[index + 1] === 'from' && userWords.has(tokens[index + 2] ?? ''),
  )
...
export class CompanionReducer {
  ...
  #resumeAfterTool(record, event) {
    ...
    const nextPayload = {
      phase: 'tool-result',
      activity: next === CompanionState.WORKING ? toolActivity(record.openTools.values().next().value) : undefined,
      stage:
  ...
  #todo(record, event) {
    ...
    return this.#withTasks([
      createMessage(CompanionMessageKind.TASK, {
        sessionId: record.id,
        sourceSeq: event.seq,
        task: record.task,
  ...
  #taskMessages() {
    ...
    const signature = tasks
      .map((task) =>
        [task.sessionId, task.state, task.project ?? '', task.task ?? '', task.message ?? '', task.detail ?? ''].join(
          '|',
        ),
      )
      .join('~')
