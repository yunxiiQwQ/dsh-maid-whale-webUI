 ...
 * bundling preset. The card is purely decorative: if DSH ever changes the slot
 * contract, it must fail quietly instead of failing the whole WebUI load.
 */
    ...
    ...BUBBLE_STATE_OPTIONS.map(([state, label]) =>
      h(
        ...
        h('input', {
          ...
          disabled,
          onChange: (event: React.ChangeEvent<HTMLInputElement>) => toggle(state, event.target.checked),
        }),
        label,
      ),
    ),
...
function CompanionCard(): React.ReactElement {
  const [status, setStatus] = useState<'loading' | 'ready' | 'unavailable'>('loading')
  const [value, setValue] = useState<CompanionConfig>({})
        ...
        : h(
            ...
            h(
              ...
              h('input', {
                ...
                disabled: !writable,
                onChange: (event: React.ChangeEvent<HTMLInputElement>) => void write('enabled', event.target.checked),
              }),
            ),
            h(
              Field,
...
interface SlotsContext {
  slots?: {
    register?: (options: Record<string, unknown>, component: () => React.ReactElement) => unknown
    inject?: (slot: string, callback: () => void) => unknown
  }
}

/** Register the companion settings card; any slot-contract failure stays local to this card. */
